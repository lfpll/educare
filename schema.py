...
class Define_type:
	def __init__(self,min_year:int = 1900):
		...
		self.assigned_val = {2:'INTEGER',3:'FLOAT',4:'DATETIME',1:'BOOLEAN',5:'STRING'}
	...
	def classify_row(self,row_val:str):
		...
		Returns:
			[type] -- [description]
		"""
		if isinstance(row_val,int) or (isinstance(row_val,str) and self.int_regex.search(row_val)):
			return 2
		elif isinstance(row_val,float) or (isinstance(row_val,str) and self.float_regex.search(row_val)):
			return 3
		elif isinstance(row_val,str) and self.date_regex.search(row_val):
			return 4
		elif isinstance(row_val,bool) or (isinstance(row_val,str) and self.bool_regex.search(row_val)):
			return 1
...
def treat_dataframe(csv_file_path:str,delimiter:str='|'):
	...
	with open(csv_file_path,'r') as csv_file_in:
		...
		with open(csv_file_path.replace('.csv','_tmp.csv'),'w') as csv_file_out:
			csv_out = csv.DictWriter(csv_file_out,delimiter=delimiter,fieldnames=header)
...
def gen_distinct_rows(csv_file_path:str):
	"""
	...
	Returns:
		list_unq [list] -- list of sets with unique value of the CSV columns
...
def classify_csv(csv_path):
	...
	tup_type = list(map(lambda column_tup: (indexed_header[column_tup[0]][1],column_tup[1]) ,columns_types))
	tup_bool_type = list(map(lambda bool_type: (indexed_header[bool_type[0]][1],'BOOLEAN'),columns_bools))
